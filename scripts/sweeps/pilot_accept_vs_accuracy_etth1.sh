...
echo "=== SWEEP START $(date) ===" >> result_inference_summary.txt

# Fixed sigma values
for S in 0.5 1.0 2.0; do
  MODEL_ID="ETTh1_specK3_fixed_s${S//./p}"
  echo "[RUN] $MODEL_ID" | tee -a result_inference_summary.txt
  CUDA_VISIBLE_DEVICES=0 python -u run.py "${COMMON_ARGS[@]}" --model_id "$MODEL_ID" --spec_sigma_mode fixed --spec_sigma "$S" --des "sweep_fixed_sigma_$S" | cat
done

# Adaptive sigma multipliers
for C in 2 4 8; do
  MODEL_ID="ETTh1_specK3_adapt_c${C}"
  echo "[RUN] $MODEL_ID" | tee -a result_inference_summary.txt
  CUDA_VISIBLE_DEVICES=0 python -u run.py "${COMMON_ARGS[@]}" --model_id "$MODEL_ID" --spec_sigma_mode adaptive --spec_sigma_adapt_c "$C" --des "sweep_adapt_c_$C" | cat
done

echo "=== SWEEP END $(date) ===" >> result_inference_summary.txt
