...
# Draft model checkpoint (0.5x scaling - d_model=512, ~2.2M params)
# NOTE: This is el1 (not el2), which gives the correct 0.5x scaling
DRAFT_TEST_DIR="forecast_ETTh1_draft_timer_xl_draft_MultivariateDatasetBenchmark_sl672_it96_ot96_lr5e-05_bt32_wd0_el1_dm1024_dff2048_nh8_cosTrue_test_0"
...
echo "[2/2] Speculative test with 0.5x draft: timer_xl + timer_xl_draft"
python -u run.py \
  --task_name forecast \
  --is_training 0 \
  --root_path ./dataset/ETT-small/ \
...
if [ -f result_inference_summary.txt ]; then
  echo "Acceptance (speculative, 0.5x draft):"
